"""CLI command and async helper for the ``compare`` backtest command.

Run all strategies on the same data and display a ranked comparison
table, optionally rendering an interactive comparison chart.
"""
...
from trading_tools.apps.backtester.cli._output import render_compare_charts
    ...
    *,
    ...
    try:
        ...
        if chart or chart_output is not None:
            render_compare_charts(results, _chart=chart, chart_output=chart_output)
    finally:
