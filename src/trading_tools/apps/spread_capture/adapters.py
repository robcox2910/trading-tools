...
class LiveMarketData:
    """Fetch live market data from Polymarket, Binance, and whale DB.

    Wrap the ``MarketScanner`` for opportunity discovery, the
    ``PolymarketClient`` for order books, the ``BinanceClient``
    for candle data, and optionally a ``WhaleRepository`` for
    whale directional signal queries.
    ...
    Attributes:
        ...
        whale_repo: Optional whale trade repository for copy-trading signals.
    ...
    async def get_whale_signal(
        ...
        Return the outcome side where tracked whales have the largest
        total BUY dollar volume since ``since_ts``.
