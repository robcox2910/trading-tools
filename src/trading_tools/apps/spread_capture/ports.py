"""Protocol interfaces (ports) for the spread capture engine.

Define the I/O boundaries that the ``SpreadEngine`` depends on.
...
class FillResult:
    """Outcome of an attempted fill on one side of a spread.
...
class ExecutionPort(Protocol):
    ...
    async def execute_fill(
        ...
        Args:
            ...
            side: ``"BUY"`` (always buying in spread capture).
...
class MarketDataPort(Protocol):
    ...
    async def get_opportunities(
        ...
        """Scan for new market opportunities, excluding already-open markets.
    ...
    async def get_binance_candles(
        ...
        """Fetch Binance 1-min candles for momentum signal computation.
    ...
    async def resolve_outcome(
        ...
        Args:
            opportunity: The spread opportunity with window timestamps
                and asset metadata.
    ...
    async def get_whale_signal(
        ...
        Returns:
            ...
            bet, ``None`` if no whale activity or no clear bias.
